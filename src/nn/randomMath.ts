...
export const sincosfResult = { sin: 0, cos: 0 };
 ...
 */
export function sincosfPair(y: number): void {
  const bits = floatBits(y);
  const top = (bits >>> 20) & 0x7ff;
  if (top < 0x3f4) {
    const x2 = y * y;
...
export function sinf(y: number): number {
  return sincosf(y, false);
}

/** glibc ``cosf`` for a float32 argument. */
export function cosf(y: number): number {
